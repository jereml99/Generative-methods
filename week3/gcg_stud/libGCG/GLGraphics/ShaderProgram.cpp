...
namespace GLGraphics {
    ...
    const std::vector<std::string>& get_generic_attrib_vec() {
        static vector<string> generic_attrib_vec = {"vertex", "normal", "texcoord"};
        return generic_attrib_vec;
    }
    ...
    void ShaderProgram::link()
    {
        for(int i=0;i<attrib_vec.size();++i) {
            glBindAttribLocation(prog, i, attrib_vec[i].c_str());
        }
    ...
    GLuint ShaderProgram::load_a_shader(GLenum shader_type, const string& shader_path, const string& str)
    {
        ...
        {
            string shaderSource = read_text_file(shader_path, str);
