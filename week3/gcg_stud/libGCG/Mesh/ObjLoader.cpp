...
#include "ObjLoader.h"
#include "../GLGraphics/read_text_file.h"
...
namespace Mesh {
    ...
    vector<Material> load_material(const string& path, const string& filename){
        const string materialSource = GLGraphics::read_text_file(path, filename);
                ...
                switch(token[0])
                {
                        ...
                        switch(token[1])
                    {
                        case 'd':
                            ...
                            break;
                        case 's':
                            materials[nummaterials].specular = Vec4f(to_vec3(material_file),0.0);
                            break;
                        case 'a':
                            materials[nummaterials].ambient = Vec4f(to_vec3(material_file),0.0);
                            break;
                    }
                        break;
                    case 'T':
                    ...
                    {
                        case 'f':
                            materials[nummaterials].transmission = Vec4f(to_vec3(material_file),0.0);
                            break;
                    }
                        break;
                    case 'i':
                    ...
                    case 'm': // Map ... all maps are treated equally.
                        if (material_file.good() && token == "map_Kd"){
                            ...
                            material_file >> name;
                            if (name != "") {
                                cout << "Loading texture: " << path << " / <"  << name << ">" << endl;
                                //                            string texname = GLGraphics::read_text_file(path, name);
                                if (materials[nummaterials].tex_map.load(path+"/"+name)) {
    ...
    bool ObjLoader::load_object(const string& path,
        ...
        vector<Vec2f> uvs;
        string resource = GLGraphics::read_text_file(path, filename);
        ...
        while (ifs.good()){
            ...
            if (iss.good() && line.length()>0){
                ...
                if (token.compare("o")==0){
                    ...
                    if(materials.empty())
                    {
                        cout << "Could not find materials in " << path << " / " << matFilename << endl;
                    }
