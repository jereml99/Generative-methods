...
bool TriangleMesh::load(const string& path, const string &filename, bool do_recompute_normals){
    ...
    bool loaded = ObjLoader::load_object(path,filename,
        &outPositions,
        &outIndices,
        &outMaterials,
        &outNormal,
...
void TriangleMesh::map_data_to_shader_vertex_attributes(GLGraphics::ShaderProgram *shader){
    ...
    for (unsigned int i=0;i<names.size();i++){
        GLint location = -1;
        if(shader)
            location = shader->add_attrib(names[i]);
        else {
            location = ShaderProgramDraw::get_generic_attrib_location(names[i]);
        }
        if (location != -1){
            glEnableVertexAttribArray(location);
            glVertexAttribPointer(location, componentSize[i], componentType[i], GL_FALSE, stride,
                                  reinterpret_cast<const GLvoid *>(offset[i]));
        } else if (debug){
